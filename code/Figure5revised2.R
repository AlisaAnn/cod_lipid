...
###############
New_logL1 <- ggplot(codFA, aes(J_date, liver_bi, color = year_fac, fill = year_fac)) +
  theme_bw()+
  xlab("Day of Year") +
  ylab("% Liver FA") +
  #theme(axis.title.x = element_blank(),
  ...
  geom_point(data = codFA,aes(x = J_date, y = liver_bi, color = year_fac))
...
New_logM1 <- ggplot(codFA, aes(J_date, muscle_bi, color = year_fac, fill = year_fac)) +
  theme_bw()+
  xlab("Day of Year") +
  ylab("% Muscle FA") +
  #theme(axis.title.x = element_blank(),
